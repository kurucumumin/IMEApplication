    ...
    public partial class frmDebitNote : Form
    {
       ...
       // frmDayBook frmDayBookObj = null;//To use in call from frmdaybook
        ...
        public void SaveOrEdit()
        {
            try
            {
                //if (CheckUserPrivilege.PrivilegeCheck(Utils.getCurrentUser().WorkerID, this.Name, btnSave.Text))
                //{
        ...
        //        dRowOther["DebitAmountInWords"] = new NumToText().AmountWords(Convert.ToDecimal(txtDebitTotal.Text), Utils.getCurrentUser().);
        ...
        public void Save()
        {
            try
            {
                ...
                DebitNoteMaster infoDebitNoteMaster = new DebitNoteMaster();
                DebitNoteDetail infoDebitNoteDetails = new DebitNoteDetail();
                PartyBalanceSP SpPartyBalance = new PartyBalanceSP();
                PartyBalance InfopartyBalance = new PartyBalance();
                ExchangeRateSP spExchangeRate = new ExchangeRateSP();
                ...
                infoDebitNoteMaster.userId = Utils.getCurrentUser().WorkerID;
                infoDebitNoteMaster.voucherTypeId = decDebitNoteVoucherTypeId;
                infoDebitNoteMaster.financialYearId = Convert.ToDecimal(Utils.getManagement().CurrentFinancialYear.ToString());
                ...
                for (int inI = 0; inI < inRowCount - 1; inI++)
                {
                    if (dgvDebitNote.Rows[inI].Cells["dgvcmbAccountLedger"].Value != null && dgvDebitNote.Rows[inI].Cells["dgvcmbAccountLedger"].Value.ToString() != string.Empty)
                    {
                        ...
                        decLedgerId = Convert.ToDecimal(infoDebitNoteDetails.ledgerId);
                    }
                    if (dgvDebitNote.Rows[inI].Cells["dgvcmbDrOrCr"].Value != null && dgvDebitNote.Rows[inI].Cells["dgvcmbDrOrCr"].Value.ToString() != string.Empty)
                    {
                        if (dgvDebitNote.Rows[inI].Cells["dgvtxtAmount"].Value != null && dgvDebitNote.Rows[inI].Cells["dgvtxtAmount"].Value.ToString() != string.Empty)
                        {
                            ...
                            if (dgvDebitNote.Rows[inI].Cells["dgvcmbDrOrCr"].Value.ToString() == "Dr")
                            {
                                ...
                                decCredit = Convert.ToDecimal(infoDebitNoteDetails.credit);
                            }
                            else
                            {
        ...
        public void FillFunction()
        {
            try
            {
                ...
                txtVoucherNo.Text = strInvoiceNo;
                decDebitNoteSuffixPrefixId = Convert.ToDecimal(infoDebitNoteMaster.suffixPrefixId);
                decDebitNoteVoucherTypeId = Convert.ToDecimal(infoDebitNoteMaster.voucherTypeId);
                dtpVoucherDate.Value = Convert.ToDateTime(infoDebitNoteMaster.date);
                ...
                dtbl1 = SpPartyBalance.PartyBalanceViewByVoucherNoAndVoucherType(decDebitNoteVoucherTypeId, strVoucherNo, Convert.ToDateTime(infoDebitNoteMaster.date));
