...
namespace LoginForm.QuotationModule
{
    public partial class FormSaleOrderAdd : DevExpress.XtraEditors.XtraForm
    {
        ...
        List<int> enabledColumns = new List<int>(new int[] { 0, 7, 14, 21, 28, 25 });
        int sO;
        ...
        public void Disc()
        {
            ...
            if (lblsubtotal.Text != null && lblsubtotal.Text != string.Empty)
            {
                ...
                if (lblsubtotal.Text != "" && Decimal.Parse(lblsubtotal.Text) != 0 && lblsubtotal.Text != null)
                {
                    ...
                    if (txtTotalDis.Text != "")
                    {
                        dis2 = Math.Round(subtotal * Decimal.Parse(txtTotalDis.Text) / 100, 2);
                        dis2 = Convert.ToDecimal(String.Format("{0:N}", dis2));
                    }
            ...
            try
            {
                lbltotal.Text = String.Format("{0:N}", (decimal.Parse(lblsubtotal.Text) - ((subtotal * DiscounRate) / 100)));
            }
            catch { }
        ...
        dgSaleAddedItems, new object[] { true });
            ...
            dgSaleAddedItems.Columns[dgLandingCost.Index].DefaultCellStyle.Format = "C2";
            ...
            cbPaymentTerm.DisplayMember = "term_name";
            cbPaymentTerm.ValueMember = "ID";
            cbPaymentTerm.SelectedValue = (int)q.PaymentID;
            if (customer.MainContactID != null) cbWorkers.SelectedValue = (int)customer.MainContactID;
            CustomerCode.Enabled = false;
            txtCustomerName.Enabled = false;
            LowMarginLimit = (Decimal)Utils.getManagement().LowMarginLimit;
        ...
        public FormSaleOrderAdd(Customer cus, List<SaleOrderDetail> list, int sayac, SaleOrder s)
        {
         ...
         dgSaleAddedItems, new object[] { true });
            ...
            txtBillTo.Text = Convert.ToString(s.BillTo);
            txtShipTo.Text = Convert.ToString(s.ShipTo);
            fillCustomer();
            ...
            lblsubtotal.Text = String.Format("{0:N}", s.SubTotal);
            txtTotalDis.Text = s.DiscOnSubtotal.ToString();
            if (txtTotalDis2.Text == null || txtTotalDis2.Text == "") txtTotalDis2.Text = "0";
            if (s.DistributeDiscount == true)
            {
                //txtTotalDis.Text = Math.Round((Decimal.Parse(txtTotalDis2.Text) * 100) / (decimal.Parse(lblsubtotal.Text) + Decimal.Parse(txtTotalDis2.Text)), 2).ToString();
                decimal dis2 = Decimal.Parse(lblsubtotal.Text) * Decimal.Parse(txtTotalDis.Text) / 100;
                txtTotalDis2.Text = String.Format("{0:N}", dis2);

            }
            else
            {
                // txtTotalDis.Text = Math.Round((Decimal.Parse(txtTotalDis2.Text) * 100) / decimal.Parse(lblsubtotal.Text), 2).ToString();
                decimal dis2 = Decimal.Parse(lblsubtotal.Text) * Decimal.Parse(txtTotalDis.Text) / 100;
                txtTotalDis2.Text = String.Format("{0:N}", dis2);
            }
            ...
            txtExtraChanges.Text = s.ExtraCharges.ToString();
        ...
        private void CalculateTotalWeight()
        {
            ...
            foreach (DataGridViewRow item in dgSaleAddedItems.Rows)
            {
                ...
                try
                {
                    rowTotal = decimal.Parse(item.Cells[dgTotalWeight.Index].Value.ToString());
                }
                catch { }
        ...
        private void CalculateTotalNetWeight()
        {
            ...
            foreach (DataGridViewRow item in dgSaleAddedItems.Rows)
            {
                ...
                try
                {
                    rowTotal = decimal.Parse(item.Cells[dgUnitTotalNetWeight.Index].Value.ToString());
                }
                catch { }
                totalnetweight += rowTotal;
            }
            ...
            switch (dgSaleAddedItems.CurrentCell.ColumnIndex)
            {
                        ...
                        if (!String.IsNullOrEmpty(dgSaleAddedItems.Rows[satir_sayisi - 1].Cells[dgProductCode.Index].Value.ToString()))
                        {
                            ...
                            if (dgSaleAddedItems.CurrentRow.Cells[dgProductCode.Index].Value.ToString().Substring(dgSaleAddedItems.CurrentRow.Cells[dgProductCode.Index].Value.ToString().Length - 1, 1) == "P")
                            {
                                if (new Sp_Item().GetProductHistoryWithArticleNo_P_SaleOrder(dgSaleAddedItems.CurrentRow.Cells[dgProductCode.Index].Value.ToString()) != null && new Sp_Item().GetProductHistoryWithArticleNo_P_SaleOrder(dgSaleAddedItems.CurrentRow.Cells[dgProductCode.Index].Value.ToString()).Rows.Count > 0)
                                {
                                    ...
                                    {
                                        btnProductHistory.ForeColor = Color.FromArgb(32, 21, 53);
                                        btnProductHistory.Enabled = false;
                                    }
                                }
                            }
                ...
                case 16://QAUANTITY
                    ...
                    if ((Int32.Parse(CurrentRow.Cells["dgQty"].Value.ToString()) % Int32.Parse(CurrentRow.Cells["dgSSM"].Value.ToString())) == 0 && (Int32.Parse(CurrentRow.Cells["dgQty"].Value.ToString()) % Int32.Parse(CurrentRow.Cells[dgUC.Index].Value.ToString())) == 0)
                    {
                        ...
                        {
                            dgSaleAddedItems.CurrentRow.Cells[WT.Index].Style.BackColor = Color.Red;
                        }
                        else if (Convert.ToDecimal(txtGrossWeight.Text) * Convert.ToDecimal(dgSaleAddedItems.CurrentRow.Cells[dgQty.Index].Value.ToString()) > 5)
                        {
                ...
                if (!String.IsNullOrEmpty(CurrentRow.Cells[dgQty.Index].Value.ToString()))
                {
                    if (QuotationUtils.GetCost(CurrentRow.Cells["dgProductCode"].Value.ToString(), 1) != 0)
                    {
                        ...
                        if (price > 0)
                        {
                            ...
                            CurrentRow.Cells["dgTotal"].Value = String.Format("{0:N}", price);
                            //SuperdiskP değilse
                            if (!(articleNo.ToUpper().Contains('P')) && Int32.Parse(CurrentRow.Cells["dgUC"].Value.ToString()) > 1)
                            {
                                ...
                                else
                                {
                                    ...
                                    CurrentRow.Cells["dgTotal"].Value = Convert.ToDecimal(String.Format("{0:N}", (price / (decimal.Parse(CurrentRow.Cells["dgUC"].Value.ToString())))));
                                }
                            ...
                            if (CurrentRow.Cells[dgQty.Index].Value.ToString() != "")
                            {
                                ...
                                CurrentRow.Cells["dgTotal"].Value = String.Format("{0:N}", (decimal.Parse(CurrentRow.Cells["dgTotal"].Value.ToString())));
                                if (CurrentRow.Cells[dgDisc.Index].Value != null && CurrentRow.Cells[dgDisc.Index].Value.ToString() != "")
                                {
                                    CurrentRow.Cells[dgDisc.Index].Value = String.Format("{0:N}", (Decimal.Parse(CurrentRow.Cells[dgDisc.Index].Value.ToString())));
        ...
        private void GetMarginMark(int rowindex)
        {
            try
            {
                if (Decimal.Parse(dgSaleAddedItems.Rows[rowindex].Cells["dgMargin"].Value.ToString()) < LowMarginLimit)
                {
                    ...
                    dgSaleAddedItems.Rows[rowindex].Cells["dgMargin"].Style.BackColor = Color.Red;
                }
                else
                {
        ...
        private void GetMarginMark()
        {
            try
            {
                if (Decimal.Parse(dgSaleAddedItems.Rows[dgSaleAddedItems.CurrentCell.RowIndex].Cells["dgMargin"].Value.ToString()) < LowMarginLimit)
                {
                    ...
                    dgSaleAddedItems.Rows[dgSaleAddedItems.CurrentCell.RowIndex].Cells["dgMargin"].Style.BackColor = Color.Red;
        ...
        private void GetLandingCost(int Rowindex)
        {
            try
            {
                ...
                dgSaleAddedItems.Rows[Rowindex].Cells["dgLandingCost"].Value = String.Format("{0:N}", dgSaleAddedItems.Rows[Rowindex].Cells["dgLandingCost"].Value.ToString()).ToString();
            }
            catch { }
        ...
        private decimal CalculateMargin(decimal _LandingCost, decimal _Price)
        {
            ...
            return Math.Round((1 - (_LandingCost / gbpPrice)) * 100, 2);
        }
        ...
        private void CalculateSubTotal()
        {
            ...
            lbltotal.Text = String.Format("{0:N}", subtotal - dectotaldisc);
        }
        ...
        private void lblTotalExtra_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                lblTotalExtra.Text = Decimal.Parse(String.Format("{0:N}", (decimal.Parse(lblTotalExtra.Text)))).ToString("G29");
            }
            chkVat_Checked();
        }
        ...
        private void totalDis2Change()
        {
            if (Decimal.Parse(lblsubtotal.Text) != 0 && lblsubtotal.Text != "" && lblsubtotal.Text != null)
            {
                ...
                if (CurrentDis != Decimal.Parse(txtTotalDis2.Text))
                {
                    ...
                    {
                        lbltotal.Text = String.Format("{0:N}", (Decimal.Parse(lbltotal.Text) + CurrentDis));
                        CurrentDis = Decimal.Parse(txtTotalDis2.Text);
                    }
                    lbltotal.Text = String.Format("{0:N}", (Decimal.Parse(lbltotal.Text) - CurrentDis));
                ...
                if (ControlSave())
                {
                    if (label75.Text == "Save Modification")
                    {
                        ...
                        try
                        {
                            ...
                            s.ShipTo = Convert.ToInt32(txtShipTo.Text);
                            s.BillTo = Convert.ToInt32(txtBillTo.Text);
                            ...
                            s.CurrName = (cbCurrency.SelectedItem as Currency).currencyName;
                            s.DistributeDiscount = cbDeliverDiscount.Checked;
                            s.TotalWeight = Convert.ToDecimal(lblTotalWeight.Text);
                            s.TotalNetWeight = Convert.ToDecimal(lblTotalNetWeight.Text);
                        ...
                        for (int i = 0; i < dgSaleAddedItems.RowCount; i++)
                        {
                            ...
                            if (row.Cells["dgProductCode"].Value != null)
                            {
                                ...
                                if (row.Cells[dgCost.Index].Value != null) sdi.Cost = Convert.ToDecimal(row.Cells[dgCost.Index].Value?.ToString());
                                ...
                                if (dgSaleAddedItems.Rows[i].Cells["dgUOM"].Value?.ToString() == "")
                                {
                                    if (sdi.UnitContent == 1 && sdi.SSM == 1)
                                    {
                                        sdi.UnitOfMeasure = "EACH";
                                    }
                                    else
                                    {
                                        sdi.UnitOfMeasure = "PACKET OF";
                                    }
                                }
                                else
                                {
                                    sdi.UnitOfMeasure = dgSaleAddedItems.Rows[i].Cells["dgUOM"].Value.ToString();
                                }

                                sdi.DeliveryID = (int)((DataGridViewComboBoxCell)dgSaleAddedItems.Rows[i].Cells[dgDelivery.Index]).Value;
                                
                                sdi.Manufacturer = dgSaleAddedItems.Rows[i].Cells[dgBrand.Index].Value.ToString();
                                sdi.SupplierName = dgSaleAddedItems.Rows[i].Cells[dgSupplier.Index].Value.ToString();
                        ...
                        SaleOrder soView = IME.SaleOrders.Where(quo => quo.SaleOrderNo == sO).FirstOrDefault();

                        soView.ViewSale = true;
                        IME.SaveChanges();
                    ...
                    else
                    {
                        ...
                        SaleOrder soo = IME.SaleOrders.Where(quo => quo.SaleOrderNo == sO).FirstOrDefault();
        ...
        private void ChangeCurr()
        {
            ...
            CalculateSubTotal();
            lblCurrValue.Text = Math.Round(CurrValue, 2).ToString();
        }
        ...
        {
            if (lblGrossTotal.Text != Decimal.Parse(String.Format("{0:N}", decimal.Parse(lblGrossTotal.Text))).ToString())
            {
        ...
        private void txtTotalDis_Leave(object sender, EventArgs e)
        {
            if (lblsubtotal.Text != null && lblsubtotal.Text != string.Empty)
            {
                ...
                if (lblsubtotal.Text != "" && Decimal.Parse(lblsubtotal.Text) != 0 && lblsubtotal.Text != null)
                {
                    ...
                    {
                        decimal subtotal = Math.Round(Decimal.Parse(lblsubtotal.Text) - hztotal, 2);
                        //decimal dis2 = Math.Round(subtotal * Decimal.Parse(txtTotalDis.Text) / 100,0);
                        decimal dis2 = Math.Round(((subtotal * Decimal.Parse(txtTotalDis.Text)) / 100), 2);
                        txtTotalDis2.Text = dis2.ToString();
                        lbltotal.Text = String.Format("{0:N}", ((Decimal.Parse(lblsubtotal.Text) - decimal.Parse(txtTotalDis2.Text))));
                    }
                }
                if (txtTotalMargin.Visible == false)
                {
                    CalculateTotalMarge();
                }
        ...
        //    //    txtTotalMargin.Text = String.Format("{0:N}", totalMargin / subtotal).ToString();
        //    //}
        //    //else
        //    //{
        //    //    decimal subtotal1 = 0;
        ...
        //    //    if(subtotal1!=0)txtTotalMargin.Text = String.Format("{0:N}", totalMargin1 / subtotal1).ToString();
        //    //}
...
.FirstOrDefault();
        ...
        private void cbDeliverDiscount_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (cbDeliverDiscount.Checked)
            {
                ...
                totallbl = subtotal;
                lbltotal.Text = String.Format("{0:N}", subtotal);
            }
            else
            {
            ...
            subtotal = Decimal.Parse(String.Format("{0:N}", subtotal));
            lblsubtotal.Text = String.Format("{0:N}", subtotal);
            totallbl = Decimal.Parse(String.Format("{0:N}", totallbl));
            lbltotal.Text = String.Format("{0:N}", totallbl);
        ...
        private void dgSaleAddedItemsRowChange()
        {
            CalculateSubTotal();
            calculateTotalCost();
            try { txtTotalMargin.Text = Math.Round(calculateTotalMargin(), 2).ToString(); }
            catch { }
            Disc();
        }
        ...
        private bool HasNullData()
        {
            ...
            if (txtExtraChanges.Text == null || txtExtraChanges.Text == String.Empty)
            {
        ...
        private decimal SaleSave()
        {
            try
            {
                ...
                decimal newSale = Decimal.Parse(txtSalesOrderNo.Text.Substring(2)) - 1;
                //getTotalDiscMargin();
                SaleOrder s = new SaleOrder();
                s.SaleOrderNo = Int32.Parse(txtSalesOrderNo.Text.Substring(2));
                s.SaleOrderID = IME.SaleOrders.Where(x => x.SaleOrderNo == newSale).FirstOrDefault().SaleOrderID+1;
                s.SaleDate = (DateTime)dtpDate.Value;
                s.OnlineConfirmationNo = txtOnlineConfirmationNo.Text;
                s.QuotationNos = txtQuotationNo.Text;
        ...
        private void modifyQuotation(Quotation q, string aa)
        {
            ...
            #endregion
            //buradaki yazılanların sırası önemli sırayı değiştirmeyin
            lblsubtotal.Text = String.Format("{0:N}", q.SubTotal);
            txtTotalDis2.Text = q.DiscOnSubTotal2.ToString();
            if (txtTotalDis2.Text == null || txtTotalDis2.Text == "") txtTotalDis2.Text = "0";
            if (q.DistributeDiscount == true)
            {
        ...
        private void SaleOrderMode(SaleOrder q)
        {
            ...
            if (importFromQuotation == 0)
            {
                ...
                txtCustomerName.Text = q.Customer.c_name;
                txtFactor.Text = Utils.getManagement().Factor.ToString();
                txtBillTo.Text = Convert.ToString(q.BillTo);
                txtShipTo.Text = Convert.ToString(q.ShipTo);
        ...
        private void FormSaleOrderAdd_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (e.CloseReason == CloseReason.UserClosing)
            {
                SaleOrder so = IME.SaleOrders.Where(q => q.SaleOrderNo == sO).FirstOrDefault();
        ...
        private void btnShipTo_Click(object sender, EventArgs e)
        {
            FormShipTo frm = new FormShipTo();
            frm.ShowDialog();
            frm.Dispose();

            txtShipTo.Text = frm.ShipTo.ToString();
        }

        private void btnBillTo_Click(object sender, EventArgs e)
        {
            FormBillTo frm = new FormBillTo();
            frm.ShowDialog();
            frm.Dispose();
        ...
        private void dgSaleAddedItems_DataError(object sender, DataGridViewDataErrorEventArgs e)
        {
            if (e.Exception.Message == "DataGridViewComboBox value is not valid.")
            {
                object value = dgSaleAddedItems.Rows[e.RowIndex].Cells[e.ColumnIndex].Value;
                if (!((DataGridViewComboBoxColumn)dgSaleAddedItems.Columns[e.ColumnIndex]).Items.Contains(value))
                {
                    ((DataGridViewComboBoxColumn)dgSaleAddedItems.Columns[e.ColumnIndex]).Items.Add(value);
        ...
        private void Column1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar))
            {
                ...
                {
                    dgSaleAddedItems.CurrentRow.Cells[dgProductCode.Index].Value = "";
                    dgSaleAddedItems.CurrentCell = dgSaleAddedItems.CurrentRow.Cells[dgProductCode.Index];
                }
            }
        }

        private void dgSaleAddedItems_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            e.Control.KeyPress -= new KeyPressEventHandler(Column1_KeyPress);
            if (dgSaleAddedItems.CurrentCell.ColumnIndex == 8) //Desired Column
            {
