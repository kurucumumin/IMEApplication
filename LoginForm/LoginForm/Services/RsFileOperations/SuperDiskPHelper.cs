    ...
    class SuperDiskPHelper
    {
        ...
        public bool ErrorCheck()
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                ReturnMessage ColumnError = SuperDiskPColumnCheck(_ColumnNames);


                //Eğer SuperDiskP dosyasında sütun isimlerinde hata varsa
                if (ColumnError.MessageList.Count() > 0)
                {
                    _Timer.Stop();
                    StringBuilder errorMessage = new StringBuilder();
                    errorMessage.Append("Error(s) exist in column names, Check SuperDiskP file again").AppendLine().Append("Mis-typed columns are:").AppendLine().AppendLine();
                ...
                else /*if(r.ErrorList.Count() == 0)*/
                {
                   ...
                   // _Timer.Stop();
                    ...
                    if (LineError.MessageList.Count > 0)
                    {
                        ...
                        StringBuilder errorMessage = new StringBuilder();
                        errorMessage.Append("Passed Time: " + _Timer.Elapsed.ToString(@"hh\:mm\:ss") + " sn" + "\n\n" + "Error(s) exist in item rows, Check SuperDiskP file again").AppendLine().AppendLine();
        ...
        private ReturnMessage CheckLinesForErrors(List<string> SuperDiskPLines)
        {
            ReturnMessage error = new ReturnMessage();

            string[] wordcontrol = SuperDiskPLines[0].Split('|');
            if (wordcontrol[0].Contains("P"))
            {
                for (int i = 0; i < SuperDiskPLines.Count(); i++)
                {
                    string[] lineColumns = SuperDiskPLines[i].Split('|');
        ...
        private ReturnMessage SuperDiskPColumnCheck(List<string> Columns)
        {
        ...
        public bool LoadSuperDiskPItems()
        {
            SqlCommand cmd;
